...
/* - Patch details - applied to "master" nabupc prior to porting to nabupc80

diff --git a/src/mame/nabu/nabupc.cpp b/src/mame/nabu/nabupc.cpp
index b66f85f8c78..945f146c05e 100644
--- a/src/mame/nabu/nabupc.cpp
+++ b/src/mame/nabu/nabupc.cpp
@@ -180,7 +180,7 @@ static __inline f9318_out_t f9318(f9318_in_t in)
...
@@ -216,10 +216,11 @@ void nabupc_state::nabupc(machine_config &config)
 	// Video hardware
...
@@ -443,7 +444,7 @@ void nabupc_state::io_map(address_map &map)
...
+	map(0xa0, 0xa1).rw(m_v9938, FUNC(v9938_device::read), FUNC(v9938_device::write));
...
diff --git a/src/mame/nabu/nabupc.h b/src/mame/nabu/nabupc.h
index 97ea7bd07b0..6852bea1908 100644
--- a/src/mame/nabu/nabupc.h
+++ b/src/mame/nabu/nabupc.h
@@ -12,7 +12,7 @@
...
#include "nabupc80.h" // Patched include as above
...
static INPUT_PORTS_START( nabupc80 )
	PORT_START("CONFIG")
...
// nabupc80_state created to seperate code from stock core

nabupc80_state::nabupc80_state(const machine_config &mconfig, device_type type, const char *tag)
	: driver_device(mconfig, type, tag)
	, m_maincpu(*this, "maincpu")
	, m_v9938(*this, "v9938")
	, m_screen(*this, "screen")
...
// nabupc8 config created to seperate code from stock core

void nabupc80_state::nabupc80(machine_config &config)
{
	config.set_default_layout(layout_nabupc);

	// CPU
	Z80(config, m_maincpu, 10.738635_MHz_XTAL / 3);         // 3.579545 MHz
	m_maincpu->set_addrmap(AS_PROGRAM, &nabupc80_state::memory_map);
	m_maincpu->set_addrmap(AS_IO, &nabupc80_state::io_map);
	m_maincpu->set_irq_acknowledge_callback(FUNC(nabupc80_state::int_ack_cb));
	...
	// F18a 80-column support (Patched)
	V9938(config, m_v9938, 21.477272_MHz_XTAL);
	m_v9938->set_screen_ntsc(m_screen);
	m_v9938->set_vram_size(0x4000);
	m_v9938->int_cb().set(*this, FUNC(nabupc80_state::vdp_int_w));
	...
	m_ay8910->port_b_read_callback().set(FUNC(nabupc80_state::psg_portb_r));
	m_ay8910->port_a_write_callback().set(FUNC(nabupc80_state::psg_porta_w));
	m_ay8910->add_route(ALL_OUTPUTS, m_speaker, 0.3);

	// Keyboard
	I8251(config, m_kbduart, 10.738635_MHz_XTAL / 6);
	m_kbduart->rxrdy_handler().set(*this, FUNC(nabupc80_state::rxrdy_w));
	...
	m_hccauart->write_dav_callback().set(FUNC(nabupc80_state::hcca_dr_w));
	m_hccauart->write_tbmt_callback().set(FUNC(nabupc80_state::hcca_tbre_w));
	m_hccauart->write_fe_callback().set(FUNC(nabupc80_state::hcca_fe_w));
	m_hccauart->write_or_callback().set(FUNC(nabupc80_state::hcca_oe_w));
	...
	m_centronics->busy_handler().set(FUNC(nabupc80_state::centronics_busy_handler));
	...
	m_bus->out_int_callback<0>().set(FUNC(nabupc80_state::j9_int_w));
	m_bus->out_int_callback<1>().set(FUNC(nabupc80_state::j10_int_w));
	m_bus->out_int_callback<2>().set(FUNC(nabupc80_state::j11_int_w));
	m_bus->out_int_callback<3>().set(FUNC(nabupc80_state::j12_int_w));
...
void nabupc80_state::machine_reset()
{
	m_irq_in_prio = 0xFF;
	m_int_lines = 0x40;
	m_porta = 0;
...
void nabupc80_state::machine_start()
{
	m_leds.resolve();
...
uint8_t nabupc80_state::psg_portb_r()
{
	return m_portb;
}

void nabupc80_state::psg_porta_w(uint8_t data)
{
	if (data != m_porta) {
		m_porta = data;
		update_irq();
...
void nabupc80_state::control_w(uint8_t data)
{
	m_control = data;
...
void nabupc80_state::centronics_busy_handler(uint8_t state)
{
	BIT_SET(m_portb, 4, state);
}

WRITE_LINE_MEMBER(nabupc80_state::hcca_fe_w)
{
	BIT_SET(m_portb, 5, state);
}

WRITE_LINE_MEMBER(nabupc80_state::hcca_oe_w)
{
	BIT_SET(m_portb, 6, state);
}

WRITE_LINE_MEMBER(nabupc80_state::hcca_dr_w)
{
	BIT_SET(m_int_lines, 7, state);
	update_irq();
}

WRITE_LINE_MEMBER(nabupc80_state::hcca_tbre_w)
{
	BIT_SET(m_int_lines, 6, state);
	update_irq();
}

WRITE_LINE_MEMBER(nabupc80_state::rxrdy_w)
{
	BIT_SET(m_int_lines, 5, state);
	update_irq();
}

WRITE_LINE_MEMBER(nabupc80_state::vdp_int_w)
{
	BIT_SET(m_int_lines, 4, state);
	update_irq();
}

WRITE_LINE_MEMBER(nabupc80_state::j9_int_w)
{
	BIT_SET(m_int_lines, 3, !state);
	update_irq();
}

WRITE_LINE_MEMBER(nabupc80_state::j10_int_w)
{
	BIT_SET(m_int_lines, 2, !state);
	update_irq();
}

WRITE_LINE_MEMBER(nabupc80_state::j11_int_w)
{
	BIT_SET(m_int_lines, 1, !state);
	update_irq();
}

WRITE_LINE_MEMBER(nabupc80_state::j12_int_w)
{
	BIT_SET(m_int_lines, 0, !state);
	update_irq();
}

IRQ_CALLBACK_MEMBER(nabupc80_state::int_ack_cb)
{
	uint32_t vector = m_portb  & 0xe;
	return vector;
}

void nabupc80_state::update_irq()
{
...
uint8_t nabupc80_state::read_mem(offs_t offset)
{
...
void nabupc80_state::memory_map(address_map &map)
{
	map(0x0000, 0xffff).r(FUNC(nabupc80_state::read_mem)).w(m_ram, FUNC(ram_device::write));
}

void nabupc80_state::io_map(address_map &map)
{
	map.unmap_value_high();
	map.global_mask(0xff);
	map(0x00, 0x00).w(FUNC(nabupc80_state::control_w));
...
/* NABUPC80 */
COMP(1984, nabupc80,   0,        0,     nabupc80,   nabupc80,      nabupc80_state, empty_init, "NABU", "NABU PC 80", 0)
